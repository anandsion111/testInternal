...
public class DeviceInfo {
    ...
    public func getidentifier()->(Any){
        let identifier = "\(getAppName())_\(Constants.DEVICE_PREFIX)\(getImei())\(Constants.DEVICE_POSTFIX)"
        return identifier
    }
    ...
    public func getDeviceId()->(Any){
      //  print("Random Number = ", generateIMEI() )
        let imei = "\(Constants.DEVICE_PREFIX)\(getDeviceuuid())\(Constants.DEVICE_POSTFIX)"//"1826314667992232"
        return imei
    }
    ...
    public func getPlatform()->(Any){
        let result = "\(Constants.APP_PLATFORM)"
        return result
    }
